...
int conectarBase(string arquivo, Jogador vetor[], int tamanho){
    ...
    while (getline(procuradorArquivo, linha)) {
        ...
        // Garante que a linha nao e vazia ou apenas espacos
        ...
        try {
            ...
            // Este bloco captura o erro 'stod' e o ignora, pulando o registro
        }
    } 
    procuradorArquivo.close();
...
int removerUsuarioPorTag(Jogador vetor[], int qtdJogadores, const string& tagParaRemover, const string& baseDados){
    int indiceEncontrado = -1;

    // 1. Encontrar o índice
    for(int i=0; i < qtdJogadores; i++){
        if(vetor[i].tag == tagParaRemover){  
            indiceEncontrado = i;
            break;
        } 
    }
...
void pesquisarJogador(Jogador vetor[], int qtdJogadores){
    ...
    for(int i=0; i < qtdJogadores; i++){
        if (vetor[i].username.find(termoBusca) != string::npos || vetor[i].tag == termoBusca) {
            cout << "ENCONTRADO na posicao " << i << ": User: " << vetor[i].username << ", Tag: " << vetor[i].tag << ", Level: " << vetor[i].level << endl;
            encontrado = true;
        }
    }
